...
> **Status: ALL FOUR STAGES HAVE LANDED. This is a completed plan, not a plan.**
> Stage 1 → `extract-actor-mechanics-builders`, stage 2 → `extract-compendium-search`,
> stage 3 → `extract-actor-crud`, stage 4 → `extract-character-reading`. Nothing here
> is still to be done; the document is kept for its corrections and for the record of
> what each stage got wrong. **Stage 4's section records three claims this document made
> about character reading that are wrong** — its size, its coverage, and a prescribed
> comment edit that would have named the wrong file and violated the
> load-bearing-comment requirement. `data-access.ts` is now ~1,520 lines, not ~7,000 and
> not ~4,270. The current inventory is `refactor-data-access.md`; read that first.
>
> This was written as a plan and is now part plan, part record. **Stage 1** (actor-mechanics builders) landed as
...
- **Facade members left behind**: six thin delegations for the externally-reached
  ...
  **permanent**, while `auditLog`/`findActorByIdentifier` are down to one caller each
  and expire at stage 4. Same shape, three different lifetimes. **All three predictions
  held exactly**: stage 4 deleted `auditLog`, `findActorByIdentifier` and `sanitizeData`
  in the commits that moved their last callers, and `getOrCreateFolder` survives. Stage 4
  then found a **fourth** lifetime this list does not contain:
  `validateFoundryState` ended the pass with **zero** intra-class callers and is
  nevertheless permanent, because `queries.ts` reaches it directly. So "nothing inside
  the class calls it any more" joins the list of readings that look like expiry and are
  not. And the wrapper deletion left one thing behind that no gate caught:
  `CompendiumSearch.findBestCompendiumMatch` was widened for that wrapper and never
  re-narrowed, so it is `public` and reached by nothing — see the follow-ups table in
  `refactor-data-access.md`, and note that the remedy is deletion rather than restoring
  `private`, which does not compile.
...
### Stage 4 — Character reading (cluster B + B′) → `character-reader.ts` — ✅ **LANDED** as `extract-character-reading`
...
**What landed, and the three things this section got wrong.** Both deferral reasons
expired: the concurrent change landed as `03a6836`, and the seam was settled by
`extract-character-reading` with an argument rather than a hint (see the resolved
deferral row in `refactor-data-access.md`). `readActorFlags` and `extractTokenArt` went
with **character reading**, and the "pending confirmation" is discharged. The
corrections:

1. **The cluster is 11 members / 995 body lines, not "B 9 / ~969" plus a separate "B′
   3 / ~43".** The nine are **970** body lines (so "~969" was one line out, which is
   fine) and `readActorFlags` + `extractTokenArt` are **25**. The **"43" counted
   `findActorsByFlag`'s 18-line facade delegation in with them** — a member that is not
   this cluster's, whose implementation was already in `actor-directory.ts`, and which
   did not move. There is no "B′": splitting the two helpers out as a fifth cluster is
   what made their home look undecided for four documents.
2. **"The best-tested cluster in the file besides `importActors`" is false, and it
   inverts.** `actor-read-path.test.ts` has 17 cases, of which **eight exercise
   `ActorDirectory.findActorsByFlag`** and reach no cluster-B code at all; the nine that
   reach `getCharacterInfo` are, without exception, about the opt-in extras. So the
   coverage this note was thinking of is coverage of `readActorFlags` and
   `extractTokenArt` — **25 of the cluster's 995 lines**, and by coincidence exactly the
   two members whose home was the open question. **Eight of eleven members had zero
   coverage: 805 body lines, 81% of the cluster**, including both four-way `systemId`
   dispatches and all three limit-truncation sites. The claim inverted the risk it was
   meant to measure, and it is the reason the pass needed a characterization change as a
   precondition (`8d14064`: 142 cases, 112 mutations attempted, 112 caught, plus pf2e-,
   dnd5e- and dsa5-shaped actor factories the shared fixture did not have).
3. **The prescribed "fix mechanically at move time" for `readActorFlags`'s comment was
   NOT applied, and must not be.** See the load-bearing-comments table below, which is
   annotated in place.
...
| `2452-2461`: "Read via raw property access... NEVER `actor.getFlag(...)`... **Same rule as `importActors`' `findBySourceId`**" | `readActorFlags`                                                                                                                                                                            | Stage 4 (deferred)                                                                                                                              | **Yes — the only real hazard in this inventory.** This comment cross-references `importActors` by name. If `readActorFlags` moves into `character-reader.ts` while `importActors` stays on the facade (or moves into a different `actor-crud.ts`), the sentence still reads correctly in prose but no longer names a file, so a future reader can't find the sibling rule without knowing both files exist. ~~**Fix mechanically at move time**: rewrite to name the actual destination file, e.g. "Same rule as `actor-crud.ts`'s `importActors`." Do this in the same diff that moves `readActorFlags`, not as a follow-up.~~ **WRONG — do NOT apply this. `extract-character-reading` moved the comment verbatim.** The instruction fails three ways. (1) `importActors` **never moved**: it is a recorded permanent deferral and is still in `data-access.ts`, so the prescribed text would name a file that does not contain it. (2) The comment names **no file** today — its text is a symbol reference, "Same rule as \`importActors\`' \`findBySourceId\`." — so the staleness described here ("no longer names a file") never existed to be fixed. (3) Editing it at all is what the load-bearing-comment requirement forbids: such a comment SHALL move **unedited**, and the per-member body diff turns that into a mechanical check, because an edited comment line is a differing line no re-pointing produced. The residual — a symbol reference across a file boundary that names no file — is **recorded** in `refactor-data-access.md` rather than repaired inside a relocation. `actor-directory.ts:165`, which genuinely does name `data-access.ts`'s `importActors`, is **correct as it stands** and was not touched. |
...
7. ~~**Stage 4** (character reading) — **the only stage left.** — code-risk is actually moderate-low (best test coverage in the file besides `importActors`), but **currently has the highest _process_ risk** because it's mid-edit by someone else. Not a code problem, a sequencing problem.~~ **LANDED, and the coverage half of this was inverted:** 81% of the cluster had no coverage at all. The code-risk verdict "moderate-low" was nonetheless right, but for a different reason than the one given — the cluster has no dead member, no misfiled neighbour and no internal deferral, alone among the four. The process risk was real and expired when `03a6836` landed.
...
touching" in three documents. Stage 4 decides its home on the merits. **It did:** `character-reader.ts` does not want it (it reads no actor and returns a description of the game system), and `extract-character-reading` gave it and the previously unclassified `getWorldInfo` a decided home of their own in a `world-metadata.ts` to be created by a following change.
