...
export class ActorManagementTools {
   ...
   *   Number shorthand routes into the first (primary) speciality.
   *   { guncombat: 3 }  →  { specialities: { archaic: {value:3,trained:true} } }
   *   Object with specialities: passed as-is (adding id causes DataModel corruption).
   *   Object without specialities: id added (level unchanged by system for spec-skills).
   */
    ...
    const normalizeSkillEntry = (sk: string, sv: any): any => {
      ...
      if (typeof sv === 'number') {
        if (isSpecSkill) {
          ...
          // mgt2e uses the stored parent value for display; no id to avoid DataModel corruption.
          return {
            value: sv,
            trained: sv > 0,
  ...
  private async handleUpdate(args: any): Promise<any> {
    ...
    const normalizedUpdates =
      gameSystem === 'mgt2e'
        ? updates.map(u =>
            u.system !== undefined ? { ...u, system: this.normalizeMGT2eSkillKeys(u.system) } : u
          )
        : updates;
